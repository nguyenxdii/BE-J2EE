...
public class AdminDepositListingController {
    ...
    public AdminDepositListingController(DepositListingRepository depositListingRepository, DepositListingService depositListingService, NotificationService notificationService, EmailService emailService, UserRepository userRepository) {
    ...
    @GetMapping
    public ResponseEntity<AppApiResponse<List<DepositListingResponse>>> getAllListings() {
        return ResponseEntity.ok(AppApiResponse.success("Lấy danh sách bài đăng thành công", depositListingService.getAllListingsForAdmin()));
    }
    ...
    public ResponseEntity<AppApiResponse<Void>> cancelListing(
        ...
        userRepository.findById(listing.getSellerId()).ifPresent(u -> {
            emailService.sendEmail(u.getEmail(), "Thông báo gỡ bài đăng", 
